...
contract testSuite is testSuite_template {
    ...
    function canSetMemberRegistry() public {
        BubbleCommunityImplementation newReg = new BubbleCommunityImplementation();
        token.setMemberRegistry(newReg);
        Assert.equal(address(token.getMemberRegistry()), address(newReg), "new registry incorrect");
        token.setMemberRegistry(community);
        Assert.equal(address(token.getMemberRegistry()), address(community), "registry incorrect");
    }
    ...
    function tryToCallMintWithoutMinterRole() public {
    ...
    function tryToCallMintBatchWithoutMinterRole() public {
        Mint[] memory batch = new Mint[](1);
    ...
    function checkMinterCanClose() public {
